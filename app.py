...
# Es importante que el orden y la cantidad de columnas coincidan con las que se usaron para entrenar el escalador
original_features = ['radius_mean', 'texture_mean', 'perimeter_mean', 'smoothness_mean',
...
for feature in original_features:
    # Para simplificar, asumimos que todos los inputs son numéricos
    input_data[feature] = st.number_input(f"Ingrese el valor para {feature}", value=0.5, format="%.4f")
...
2.  Asegórate de que los archivos `scaler.joblib` y `ensemble_voting_original_model.joblib` estén en la misma carpeta.
